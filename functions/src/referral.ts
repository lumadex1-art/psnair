 ...
 */
export const getReferralHistory = async (request: CallableRequest<{}>) => {
  if (!request.auth) {
    throw new HttpsError("unauthenticated", "User must be authenticated");
  }
  ...
  try {
    ...
    for (const doc of referralsSnapshot.docs) {
      ...
      const referredUserDoc = await db.collection("users")
        .doc(referralData.referredUid)
        .get();
      ...
      referralHistory.push({
        ...
        referredUser: {
          ...
          name: referredUserData?.displayName || referredUserData?.name || "User",
          joinedAt: referralData.completedAt,
        },
        reward: {
          amount: referralData.bonusAmount,
          currency: "EPSN",
          claimedAt: referralData.completedAt,
        },
    ...
    const thisMonthEarnings = referralHistory
      .filter(item => item.referredUser.joinedAt.toDate() >= thisMonth)
      .reduce((sum, item) => sum + item.reward.amount, 0);
    
    const lastMonthEarnings = referralHistory
      .filter(item => {
        const joinDate = item.referredUser.joinedAt.toDate();
